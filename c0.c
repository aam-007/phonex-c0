/*
 * Phonex-C0: Tuned Performance Version
 * ------------------------------------
 * Optimized for convergence on toy datasets.
 *
...
#define D_MODEL 64
#define D_FF 128
#define SEQ_LEN 64        // Increased to fit full sentence comfortably
#define VOCAB_SIZE 128
#define EPSILON 1e-5f      // Tighter epsilon for better precision
#define CLIP_THRESHOLD 5.0f // Looser clipping to allow learning
#define LR 0.01f           // Aggressive LR for SGD
...
void tensor_init_xavier(Tensor* t) {
    // [TUNED] Standard Xavier (removed the 0.5 dampener)
...
int main() {
    ...
    printf("Training Phonex-C0 (Tuned)...\n");
    
    // [TUNED] Increased to 5000 steps to ensure convergence
    for(int step=0; step<5000; step++) {
        ...
        int start = rand() % (data_len - batch_len - 1);
        for(int i=0; i<batch_len; i++) {
            inputs[i] = (int)text[start+i];
            targets[i] = (int)text[start+i+1];
        }
        ...
        if(step % 500 == 0) printf("Step %d \t Loss: %.4f\n", step, loss);
